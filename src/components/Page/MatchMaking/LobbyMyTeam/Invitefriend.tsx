...
const InviteFriend = ({ teamID, betAmount, setInvite }: Props) => {

    const [isSent, setSent] = useState(false);
    const [activeData, setActiveData] = useState({ team_id: teamID, activePage: 1, totalPage: 1, limit: 10, search: "" });
    ...
    useEffect(() => {
      ...
      const getInviteFriends = (data: any) => {
        UserAPI.getInviteFriendListAPI(data)
          .then(res => {
            setActiveData({ team_id: teamID, activePage: activeData.activePage, totalPage: res.data.totalCounts, limit: activeData.limit, search: activeData.search });
            if(res.data.success){
    ...
    const onSendInvitation = async (recID: any, position: number) => {
        ...
        MessageAPI.sendInvitation(data2)
            .then(res => {
                if(res.data.success){
        ...
        return (
            <div className=" flex justify-between items-center">
                ...
                <div className="flex items-centers">
                    ...
                    <div className="flex flex-col items-start justify-center">
                        <p className="text-[12px] mb-[5px] font-bold">{value.avatar_unique_name}</p>
                        <p className="text-[11px] font-medium text-slate-600">{value.is_online? "Online" : "Offline"}</p>
        ...
        <div className="relative">
            ...
            <div className="bg-[#0d1010] h-full font-light p-6 overflow-y-auto text-white  " style={{
                ...
                <p className="text-[12px] mb-[10px] uppercase mt-6">
                    Offline friends ({offlineFriendData.length})
                </p>
