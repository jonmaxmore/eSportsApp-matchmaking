...
const SelectGame = () => {
  ...
  const [activeData, setActiveData] = useState({ activePage: 1, totalPage: 1, limit: 9, search: "" });
    ...
    <div>
      ...
      <div className="flex justify-end px-16 py-6">
        {games.length > 0 && <button disabled={selected == -1}
          ...
          onClick={() => navigate(`/customgame/creatlobby`, { state: { id: selected } })}>
          confirm
        </button>}
      </div>
    </div>
