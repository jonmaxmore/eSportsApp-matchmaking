...
const Wheelspin = ({bonusAmount, spin}: any) => {
    const [mustSpin, setMustSpin] = useState(spin);
    const [prizeNumber, setPrizeNumber] = useState(bonusAmount);
    const [stopspin, setStopspin] = useState(false);
    const [bonusAmounts, setBonusAmounts] = useState([] as any);
    const [bonusFor, setBonusFor] = useState('');

    useEffect(() => {
        getBonusDetailsAndAmount();
    },[]);

    const getBonusDetailsAndAmount = () => {
        GameAPI.getBonusDetailsAndAmountAPI()
        .then(res => {
            if(res.data.success){
                setBonusAmounts(res.data.bonusAmount);
                setBonusFor(res.data.bonusFor.key_value);
            }else{
    ...
    return (
        <div className="w-[1100px] h-[600px] flex flex-col items-center relative">
            ...
            <span className="py-3 w-80 absolute 2xl:-bottom-12 text-white text-[20px] text-center uppercase font-bold">{bonusFor === '1' ? "Win Team only" : "All Team in Match"} </span>
        </div>
    )
}
